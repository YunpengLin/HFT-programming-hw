...
When the best-level quantities are balanced, OBI is zero and microprice equals mid. A rising OBI suggests stronger buying pressure at the best prices: bid quantity is increasing relative to ask quantity, which can indicate upward short-term price pressure. Here, OBI rises from `0.0000` to `0.8000` and microprice moves toward the ask, although the mid remains `100.0100` across those two snapshots. In the final snapshot, OBI falls to `-0.8000` and microprice moves toward the bid, while the mid rises to `100.0200`. OBI is a signal of current book imbalance, not a guarantee of the next price move.
